...
# NaN = Not a number (безопасные функции)
'''print(np.nanmin(a))
print(np.nanmin(a, axis=0))
print(np.nanmin(a, axis=1))
'''
# Транслирование (broadcasting)
# — набор правил, которые позволяют осуществлять бинарные операции c массивами разных форм и размеров

'''a = np.array([0,1,2])
b = np.array([5,5,5])

print(a + b)
print(a + 5)''' # '5' транслирутеся в [5,5,5], т.е. она подстраивается под размер массива a
...
'''x = [3,2,3,5,6,4,7,3,7,2]
print(sorted(x))
print(np.sort(x))'''
# print(x.sort())

'''data = np.zeros(4, dtype = ('names':('name', 'age'),'formats':('U10', '14')))
